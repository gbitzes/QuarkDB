...
rocksdb::Status StateMachine::rawScanTombstones(StagingArea &stagingArea, std::string_view key, size_t count, std::vector<std::string> &elements) {
  ...
  while(iter->Valid() && elements.size() <= count) {
    uint64_t num = DecodeFixed64(iter->key().data() + iter->key().size() - 8);
    unsigned char c = num & 0xff;
    ValueType valueType = static_cast<ValueType>(c);

    if(valueType != kTypeDeletion && valueType != kTypeSingleDeletion) {
      iter->Next();
      continue; // not a tombstone, skip
    }

    std::string_view nextKey = iter->key().ToStringView();
    nextKey.remove_suffix(8u);

    elements.emplace_back(nextKey);
    iter->Next();
  }
...
rocksdb::Status StateMachine::rawScanTombstones(std::string_view seek, size_t count, std::vector<std::string> &keys) {
